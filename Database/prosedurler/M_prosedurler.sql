...
(
    ...
    P_KAYITOLUSTURMATARIHI  OUT KB_SUBE.KAYITOLUSTURMATARIHI%TYPE
)
AS
...
(
    ...
    P_ADRES       IN  KB_SUBE.SUBEADRES%TYPE,
    P_DURUMKODU   IN  KB_SUBE.SUBEDURUMKODU%TYPE,
    P_ACILISTARIHIBASLANGIC IN KB_SUBE.KAYITOLUSTURMATARIHI%TYPE,
    P_ACILISTARIHIBITIS     IN KB_SUBE.KAYITOLUSTURMATARIHI%TYPE,
...
BEGIN
    OPEN P_SONUC FOR
        ...
        WHERE
            ...
            AND (
                P_ACILISTARIHIBASLANGIC IS NULL
                OR TRUNC(KAYITOLUSTURMATARIHI) >= TRUNC(P_ACILISTARIHIBASLANGIC)
            )
            AND (
                P_ACILISTARIHIBITIS IS NULL
                OR TRUNC(KAYITOLUSTURMATARIHI) <= TRUNC(P_ACILISTARIHIBITIS)
            )
        ORDER BY
...
BEGIN
    ...
    (
        ...
        /* Doğrulama ve damgalama alt prosedürde yapılır. */
        P_RECORDUSER           => P_RECORDUSER,
...
(
    ...
    P_RECORDUSER
        IN KB_MUSTERIILETISIM.RECORDUSER%TYPE,

    P_TELEFONNO
        IN KB_MUSTERIBILGILERI.TELEFONNO%TYPE DEFAULT NULL,

    P_EPOSTA
        IN KB_MUSTERIBILGILERI.EPOSTA%TYPE DEFAULT NULL,

    P_EVTELEFON
        IN KB_MUSTERIILETISIM.EVTELEFON%TYPE DEFAULT NULL,

    P_ISTELEFON
        IN KB_MUSTERIILETISIM.ISTELEFON%TYPE DEFAULT NULL,

    P_EVADRES
        IN KB_MUSTERIILETISIM.EVADRES%TYPE DEFAULT NULL,

    P_ISADRES
        IN KB_MUSTERIILETISIM.ISADRES%TYPE DEFAULT NULL
)
AS
...
AS
    ...
    V_RECORDUSER KB_HESAPHAREKETI.RECORDUSER%TYPE;
    V_RECORDDATE KB_HESAPHAREKETI.RECORDDATE%TYPE;
BEGIN
    KB_AUDIT_BILGI_HAZIRLA
    (
        P_RECORDUSER       => P_RECORDUSER,
